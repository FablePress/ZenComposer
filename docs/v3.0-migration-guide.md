...
This migration guide includes all information regarding changes users must be aware of when updating from Zen Composer 2.x to Zen Composer 3.0. 

Zen Composer (hereafter referred to as the 'Editor') has removed the original [CodeMirror](https://codemirror.net/), squire, and to-mark dependencies and has modified the editor to use abstract models through Prosemirror. Since the core module API and plugin usages were changed, it is advised that users consult the migration guide carefully. The table of contents is as follows and refers to the 'Changes' in the order enumerated when updating. 
...
To use the Editor, use the [scoped package](https://docs.npmjs.com/using-npm/scope.html) to install the `@fablepress/zen-composer` package as you did for the previous v2.x. The following is an example of using the npm command to install the Editor. 

```sh
$ npm install @fablepress/zen-composer
$ npm install @fablepress/zen-composer@<version>
```

#### Usages

```js
const Editor = require('@fablepress/zen-composer'); /* CommonJS */
import Editor from '@fablepress/zen-composer'; /* ES6 Module */
```
...
```js
import { EditorCore } from '@fablepress/zen-composer'; /* ES6 Module */
...
Secondly, the `theme/zen-composer-dark.css` is added for the dark theme support. The dark theme will be covered more in depth in [Added Dark Theme](#-added-dark-theme). 
...
```json
{
  "main": "dist/zen-composer.js",
  "module": "dist/esm/",
  "exports": {
    ".": {
      "import": "./dist/esm/index.js",
      "require": "./dist/zen-composer.js"
    },
    "./viewer": {
      "import": "./dist/esm/indexViewer.js",
      "require": "./dist/zen-composer-viewer.js"
    }
  }
}
```
...
#### Added Dark Theme
v3.0 ships with dark theme included. To apply the dark theme, add the `theme/zen-composer-dark.css` and set the editor's `theme` option to be `dark`. Currently, in v3.0, only the dark theme is supported, but the `theme` option was added to support more diverse combinations of themes in the future. 

```js
import Editor from '@fablepress/zen-composer';
import '@fablepress/zen-composer/dist/zen-composer.css';
import '@fablepress/zen-composer/dist/theme/zen-composer-dark.css';
...
<head>
  ...
  />
  <link rel="stylesheet" href="https://uicdn.toast.com/editor/latest/zen-composer.css" />
  ...
</head>
<body>
  ...
...
const popup = {
  ...
  popup: {
    className: 'zen-composer-popup-color',
...
From v3.0, the jQuery Wrapper has been removed. To use jQuery, the user must wrap the `@fablepress/zen-composer` package separately.
