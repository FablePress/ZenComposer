...
The first parameter of a convertor function is a `Node` type object which is the main element of the AST(Abstract Syntax Tree) constructed by the ZenMark. Every node has common properties for constructing a tree, such as `parent`, `firstChild`, `lastChild`, `prev`, and `next`.
...
The following markdown text and AST tree object will help you understand the structure of AST generated by the ZenMark.
...
The type definition of each node can be found in the [source code](https://github.com/nhn/zen-composer/blob/master/libs/zenmark/src/commonmark/node.ts).
...
The AST genereted by ZenMark will be like below. (only essential properties are specified)
...
In a normal situation, we don't need to care about formatting of converted HTML string. However, as the ZenMark support [CommonMark Spec](https://spec.commonmark.org/0.29/), the renderer supports an option to control new-lines to pass the [official test cases](https://spec.commonmark.org/0.29/spec.json).
