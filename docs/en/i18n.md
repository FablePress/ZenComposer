...
Zen Composer provides the ability to set the text set in UI in various languages. There are language files provided by default, you can import each language file and set the code ​​for the language you want to use when you create an instance.
...
Below is a table of valid language codes for i18n files provided by the Zen Composer. This language code is based on the [IETF language tag](https://en.wikipedia.org/wiki/IETF_language_tag). When you import a language file, the language code is registered and you can set the code as the option.
...
Use the `setLanguage` static method to override the value for a specific language code. See [here](https://github.com/nhn/zen-composer/tree/master/apps/editor/src/i18n/en-us.ts) for default values.
...
Refer to [this file](https://github.com/nhn/zen-composer/tree/master/src/i18n/en-us.ts) and write each parameter value used when calling the `setLanguage` method.
...
You can see the example [here](https://nhn.github.io/zen-composer/latest/tutorial-example16-i18n).
